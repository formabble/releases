...
| `Output ➡️` ||Returns true if all elements in the input are less than the specified value and false otherwise. | | [`Bool`](../../types/#bool) |
...
Checks if all elements in the input are less than the given value. It returns true if all elements are less and false otherwise.
