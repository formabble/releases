...
| `Output ➡️` ||Returns true if all elements in the input are not equal to the specified value and false otherwise. | | [`Bool`](../../types/#bool) |
...
Checks if all elements in the input are not equal to the given value. It returns true if all elements are not equal and false otherwise.
