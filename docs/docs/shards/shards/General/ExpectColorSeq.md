...
| `Output ➡️` ||Returns the input value unchanged if it is a sequence of Color vectors. | | [`[Color]`](../../types/#seq) |
...
Checks if the input value is a sequence of Color vectors. The shard returns the input value unchanged if it is of the appropriate type; otherwise, the shard will trigger an error, preventing further execution.
